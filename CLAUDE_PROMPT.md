...
| 能力 | 场景 |
|------|-----------|
| `breath` | **每次对话最开头**调用一次（不传参数）——就像睁眼看手机，看看有没有钉选的核心准则和最近的记忆。不传 `query` 时只返回钉选桶+最近记忆，不做语义浮现；有明确话题时传 `query` 关键词检索，这时才触发语义浮现，返回匹配结果。传 `domain="feel"` 读取你之前留下的 feel。`max_tokens` 控制返回总 token 上限（默认 -1 = 按模式自动：自适应检索 5000 省钱、其它 10000；显式传数字则按值，上限 20000），`max_results` 默认 -1 = 自适应（按相关度自动决定条数，token 预算兜底，不用手动调）；需要硬限时再显式传数字 |
...
### breath 的参数技巧
- 无参数：只返回钉选桶（核心准则）+ 最近记忆（按更新时间排序），不做语义浮现（每次对话开头用这个）
...
- `startup=True`：一站式启动模式，一次调用返回 核心准则+最近记忆 + Dreaming 最近记忆 + 最近 3 条 feel，对话开头用这个（同样不传 query，不做语义浮现）
...
- 钉选桶不会出现在「最近记忆」列表里（它们始终在「核心准则」板块），关键词检索时始终可达
...
- 用第一人称写，内容自由，不强求分类
- feel 不参与普通 breath 无 query 的最近记忆列表，不参与衰减，不参与 dreaming
- 用 `breath(domain="feel")` 读取你之前留下的 feel

### 对话启动完整流程
```
1. breath(startup=True) — 一次调用打包：核心准则+最近记忆 + Dreaming 最近记忆 + 最近 feel
