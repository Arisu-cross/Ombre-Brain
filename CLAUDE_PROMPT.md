...
- `startup=True`：一站式启动模式，一次调用返回 浮现（核心准则+未解决记忆）+ Dreaming 最近记忆 + 最近 3 条 feel，对话开头用这个
- `wake=True`：触发式唤醒模式，忽略其它检索参数，只返回钉选桶（核心准则）+ 最近归档的记忆（按归档时间降序，默认 3-5 条，可用 `max_results` 调整条数）；用于轻量唤醒场景，不拉未解决桶
...
### 对话启动完整流程
```
1. breath(startup=True) — 一次调用打包：浮现 + Dreaming 最近记忆 + 最近 feel
