...
### breath 的参数技巧
- 无参数：只返回钉选桶（核心准则）+ 最近归档的会话总结（按归档时间降序，默认 3-5 条，可用 `max_results` 调整），不做语义浮现（每次对话开头用这个）。普通动态桶不出现在这里，只在有 `query` 的语义搜索中被检索
...
- `startup=True`：一站式启动模式，一次调用返回 核心准则+最近归档 + Dreaming 最近记忆 + 最近 3 条 feel，对话开头用这个（同样不传 query，不做语义浮现）
- `wake=True`：触发式唤醒模式，忽略其它检索参数，只返回钉选桶（核心准则）+ 最近归档的记忆（按归档时间降序，默认 3-5 条，可用 `max_results` 调整条数）；用于轻量唤醒场景，不做语义浮现
...
### 对话启动完整流程
```
1. breath(startup=True) — 一次调用打包：核心准则+最近归档的会话总结 + Dreaming 最近记忆 + 最近 feel
2. 想完之后：值得放下的 trace(resolved=1)，有沉淀的 hold(feel=True)
3. 然后开始和用户说话
```
一次工具调用替代旧的 breath→dream→breath(feel) 三连，省 token 也省往返。
