...
| `RESURFACE_N` | 否 | `2` | `dream()` 的「旧事重提」:每次做梦额外捞几条很久没被想起的旧记忆。设 `0` = 整条通道关闭（连重提标记都不写） |
| `RESURFACE_MIN_IDLE_DAYS` | 否 | `30` | 多久没被想起才算「旧事」。太短会把上周的事当旧事翻出来 |
| `RESURFACE_COOLDOWN_DAYS` | 否 | `14` | 同一件旧事重提后多少天内不再翻出来，防止连着几轮念叨同一件事 |
| `RESURFACE_MIN_SCORE` | 否 | `4.0` | 重提的最低分量门槛，不够就一条都不提。量级参考：重要度9+情绪强+一年没想起≈17，重要度4+平淡+半年≈2.6 |
| `RESURFACE_EXCERPT_CHARS` | 否 | `200` | 旧事重提给多少字正文摘录（`0`=只给标题行）。只有标题行他认不出是哪件事，等于没重提 |
