...
2. `query` 为空/未传 → 进入**无 query 分支**：只返回钉选桶 + 最近记忆，**不做语义浮现**（语义浮现——权重排序/冷启动检测/多样性采样——只在 `query` 非空的检索分支触发，见场景 4/5 等检索场景）
...
5. 筛选最近记忆候选（排除 `permanent/feel/pinned/protected`，应用 `date_from/date_to`、`include_dormant` 过滤；**不再按 `resolved` 过滤，也不做冷启动检测/权重排序**）
6. 按 `last_active`（缺失时回退 `created`）**降序**排列
7. 截断到生效条数：`max_results=-1`（默认）→ 取前 15；显式传 `>=1` 则硬截断为该值（最大 50）
...
2. 内部依次组装三个板块，任一板块失败只跳过该板块，不影响其余：
   - **核心准则 + 最近记忆**：复用无参 `breath()` 的无 query 分支（钉选桶 + 按更新时间排序的最近记忆，不做语义浮现），子预算为 `max_tokens - 4000`（下限 3000）
...
| 场景 | 异常情况 | 降级行为 |
|------|---------|---------|
| `breath()` 无 query | 桶目录为空 | 返回 `"权重池平静，没有需要处理的记忆。"` |
