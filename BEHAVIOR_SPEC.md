...
2. `query` 为空/未传 → 进入**无 query 分支**：只返回钉选桶 + 最近归档的会话总结，**不做语义浮现**（语义浮现——权重排序/冷启动检测/多样性采样——只在 `query` 非空的检索分支触发，见场景 4/5 等检索场景）。普通动态桶（`hold` 写入的）**不出现**在这里，只作为语义搜索的检索库存在
3. `bucket_mgr.list_all(include_archive=True)` — 遍历 `permanent/` + `dynamic/` + `feel/` + `archive/` 目录，加载所有 `.md` 文件的 frontmatter + 正文
4. 筛选钉选桶（`pinned=True` 或 `protected=True`）
5. 筛选归档桶（`type=="archived"`，即 `archive_session` 写入归档区的会话总结；应用 `date_from/date_to`、`include_dormant` 过滤）
...
**设计意图**：每次新窗口唤醒，读到的应该是"上一个/前几个窗口的会话总结"（`archive_session` 写入），而不是被更新顶到前面的老动态桶

**注意**：此分支**不调用** `touch()`，不重置衰减计时器

---

### 场景 2：新对话开始（有历史记忆，breath 返回钉选+最近归档）
...
| 场景 | 异常情况 | 降级行为 |
|------|---------|---------|
| `breath()` 无 query | 无钉选桶且归档区为空 | 返回 `"没有钉选记忆，也没有归档的会话总结。"` |
| `breath()` 无 query | `list_all()` 异常 | 返回 `"记忆系统暂时无法访问。"` |
